    ...
    const columns: GridColDef[] = [
        {
          ...
          flex: 1,
          renderCell: (params) => (
            <Button variant="text" size="small" onClick={() => openExamineeModal?.(params.row)}>
              {params.value}
            </Button>
          ),
        },
            ...
            return (
              <Stack direction="row" spacing={1}>
                {status === "Student Submission" && (
                  <>
                    <Button variant="outlined" size="small" onClick={handleResetTimer}>Reset Timer</Button>
                    <Button variant="outlined" size="small" onClick={handleRestartSession}>Restart Session</Button>
                  </>
                )}
                {status === "Absent" && (
                  <Button variant="outlined" size="small" onClick={handlePaperMode}>Switch to Paper Mode</Button>
                )}
              </Stack>
            );
          },
